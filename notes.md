...
18. other deployment options could use ecs (elastic container service) apart from ec2 instances 
...
we will also be now changing our port to 7860 where this flask app would now be listening to port 7860 as opposed to 5000 when deployed to a server

going back our requests to the server will now be `https://<hugging face username>-<hugging face space name>.hf.spaces/model-names` and `https://<hugging face username>-<hugging face space name>.hf.spaces/send-data`
